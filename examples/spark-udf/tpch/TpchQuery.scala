...
object TpchQuery {
  ...
  def executeQueries(sc: SparkContext, schemaProvider: TpchSchemaProvider, queryNum: Int): ListBuffer[(String, Float)] = {
    ...
    for (queryNo <- fromNum to toNum) {
      ...
      stageMetrics.begin()
      val outDF = outputDF(query.execute(sc, schemaProvider), OUTPUT_DIR, query.getName())
      stageMetrics.end()
  ...
  def main(args: Array[String]): Unit = {
    ...
    //Note: comment out local, use belwo HDFS instead
    val INPUT_DIR = "file://" + new File(".").getAbsolutePath() + "/dbgen"

    // read from hdfs
    // val INPUT_DIR: String = "/dbgen"
    val INPUT_DIR = "hdfs://10.124.48.67:9000/tpch_data100"
